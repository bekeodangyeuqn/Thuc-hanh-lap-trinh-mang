        ...
        else
        {
            ...
            while (countLogin < 3)
            {
                ...
                else if (temp->status == 1)
                {
                    ...
                    while (1)
                    {
                        ...
                        if (strcmp(buffer, "bye") == 0)
                        {
                            ...
                            sendto(sockfd, (const char *)bye, strlen(bye), MSG_CONFIRM, (const struct sockaddr *)&client_address, len);
                            logged = NULL;
                            break;
                        }
                        ...
                        {
                            t = deleteNode(t, logged->username);
                            addNodeToTree(username, newPassword, logged->status, &t);
                            saveFile(t, fin);
                ...
                else if (temp->status == 0 || temp->status == 2)
                {
                    ...
                    sendto(sockfd, (const char *)ack4, strlen(ack4), MSG_CONFIRM, (const struct sockaddr *)&client_address, len);
                    break;
                }
            }
            if (countLogin >= 3)
